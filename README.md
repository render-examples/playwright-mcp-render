...
| [`render-smoke-test.sh`](./render-smoke-test.sh) | Builds the image and checks the wrapper end to end: fails closed without `MCP_TOKEN`, `401` without the header, bounded logging under a flood of bad ones, a handshake and a real browser tool call with the right one, clean `SIGTERM`, and identical behavior with Render's proxy headers present. CI runs this on every PR. |
...
- **Bounds its own logs.** Rejections log method, path, and `401` — five per minute, then a single summary line — so an attacker hammering the gate can't bury everything else in your logs. This is log hygiene, not a rate limit: there is deliberately no per-client lockout, because `MCP_TOKEN` is machine-generated and not guessable at HTTP speed, and limiting by source address belongs at Render's edge (see below). A valid token is never affected by anyone else's failures.
...
This is one door, not defense in depth. It does not sandbox the RCE, cap what an authenticated caller can do, or give you per-client identity — everyone who has the token is the same principal. It also does not rate-limit or otherwise throttle callers: a token that leaked is fully usable until you rotate it, and unauthenticated traffic is refused but not slowed. If you can, layer something stronger on top:

- **Restrict who can reach the service at all.** [Inbound IP rules](https://render.com/docs/inbound-ip-rules) scoped to your known addresses (Scale and Enterprise plans) mean a stolen token isn't usable from anywhere else. This is also where rate limiting belongs — the edge sees the real client address, which a process behind Render's proxy does not reliably get.
